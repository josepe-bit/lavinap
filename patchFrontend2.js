...
if (!code.includes('Reservar canchas</button>')) {
    const regex = /<\/div>\s*<\/div>\s*\)}\s*(?=<\/div>\s*<\/div>\s*\{\/\* Columna Derecha: Directorio de Fechas \*\/)/;
    if(regex.test(code)) {
        code = code.replace(regex, buttonsHTML);
        fs.writeFileSync(frontendPath, code);
        console.log('AdminDashboard.jsx patched successfully via regex.');
    } else {
        console.log('Regex did not match. Trying fallback string search.');
        const lines = code.split('\\n');
        lines.forEach((l, i) => { if(l.includes('Directorio de Fechas Programadas')) console.log(lines.slice(i-10, i+2).join('\\n')); });
    }
} else {
